...
namespace rokae {
  ...
  * @class BaseRobot
  * @brief 机器人通用接口
  * 使用CRTP进行接口设计，可以在编译期确定机器人类型，如果派生的类没有写覆写某个接口，则编译期会报错。典型用法如下:
  * 在基类中:使用模板参数T调用派生类的接口，如果派生类没有覆写该接口，则编译期报错，在基类中会使用static_cast<T*>(this)，把this指针转换为派生类指针，从而调用派生类的接口。
  * eg：static_cast<T*>(this)->interfaceMethod();
  */
 ...
 class XCORE_API Robot_T : virtual public BaseRobot {
  ...
  public:
   /**
    * @brief Default constructor, call connectToRobot(remoteIp) afterwards 无参数构造函数，需随后调用connectToRobot(remoteIp)
    * 看使用需求，可以选择使用默认构造函数后再连接机器人，或者直接使用带参数的构造函数创建并连接机器人实例
    * 因为使用了带参数的构造函数，所以默认构造函数需要显式声明
    */
   Robot_T() = default;
